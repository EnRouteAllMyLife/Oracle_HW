...
* Does diagnosed patients have larger frequency of hospitalizations over six months?
* Are patients with Type 1 Diabetes more likely to adhere to medication regimens, as indicated by lower scores on the Medication Adherence Scale (MMAS)? Furthermore, among Type 1 Diabetes patients, does increased non-adherence, reflected by higher MMAS scores, correlate with a higher frequency of hospitalizations? 
...
We observed that demographic variables exhibit greater differences between the diagnosed and non-diagnosed groups compared to the hospitalized and non-hospitalized groups. This raises concerns that these demographic factors may act as confounding variables in our analysis. Notice that for missing values in `BMI`, given the missing rate is relatively small and sample size is large enough, we implemented the missing value with the mean.
...
zinp_m1 <-zeroinfl(RUHP6Q~ DBDX*CCI,data= matched_data , dist="negbin") 
summary(zinp_m1)
```
