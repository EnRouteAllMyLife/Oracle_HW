...
* Compared to Type 2 Diabetes, are patients with Type 1 Diabetes more likely to adhere to medication regimens? Furthermore, among Type 1 Diabetes patients, does increased non-adherence, reflected by higher MMAS scores, correlate with a higher frequency of hospitalizations? 
...
We exclusively considered patients with confirmed diagnoses. If we calculated the mean among the two group (a little violation of variable type given `MMAS` was treated as ordinal), excluding the missing values, we noticed that Type1 patients have a slightly larger mean `MMAS` (Type1 = 2.53, Type2 = 2.33). The pie charts depict the distribution of various `MMAS` score categories, indicating a potentially marginal difference between the groups.
...
<span style="font-size: 18px;">**For Type 1 Diabetes patients, does higher MMAS lead to higher frequency of hospitalizations?**<span>
...
1.**Confounding Variables**: There might be confounding variables that are not accounted for in the analysis. These variables could influence both `MMAS` and hospitalization rates simultaneously, leading to a spurious association between `MMAS` and hospitalization.

2.**Reverse Causality**: It's possible that the relationship between `MMAS` and hospitalization is bidirectional. In other words, not only does medication adherence affect hospitalization rates, but hospitalization experiences could also influence future medication adherence. For example, individuals who have experienced frequent hospitalizations may become more vigilant about medication adherence, resulting in lower `MMAS` scores.


# Q3.

<span style="font-size: 18px;">**For Type2 DB patients, how to predict the possibility of hospitalizations in 6 months?**<span>
